...
namespace Ghost.Puzzles.ConfidenceFallback
{
    ...
    /// <summary>
    /// Which risk the player decided to live with. The vague courier can be answered or bounced and
    /// both settings pass the day, so this is what the debrief talks about instead of a score.
    /// </summary>
    ...
    {
        private readonly Act4RouteOutcome[] accepted;

        public Act4VisitorMessage(
            string id,
            string message,
            int confidenceScore,
            Act4RouteOutcome[] acceptedOutcomes,
            ...
            int arrivesInPass = 1)
        {
            ...
            if (acceptedOutcomes == null || acceptedOutcomes.Length == 0)
            {
                throw new ArgumentException("A visitor needs at least one accepted outcome.", nameof(acceptedOutcomes));
            }

            Id = id;
            Message = message;
            ConfidenceScore = confidenceScore;
            accepted = (Act4RouteOutcome[])acceptedOutcomes.Clone();
            Lines = lines ?? throw new ArgumentNullException(nameof(lines));
            SoundsUpset = soundsUpset;
            ArrivesInPass = arrivesInPass;
        }
        ...
        /// <summary>More than one entry means the player may legitimately choose between them.</summary>
        public IReadOnlyList<Act4RouteOutcome> AcceptedOutcomes => accepted;
    ...
    {
        internal Act4ShiftTally(int answered, int rephrased, int handedOff, int upset)
        {
            Answered = answered;
            Rephrased = rephrased;
            HandedOff = handedOff;
